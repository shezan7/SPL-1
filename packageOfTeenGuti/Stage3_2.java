...
public class Stage3_2 extends Pane {
	...
	public int[] greenIndex = new int[3];
			...
			if(play) {
				...
				for(int c=0;c<9;c++)
				{
					...
					{							
							...
							selectedGuti=selectTheNumberOfGuti();
							previousPosiotion=greenIndex[selectedGuti];
							guti[selectedGuti].setStroke(Color.RED);
						...
						if(firstTeamClick2)
						{							
							selectedPosition=selectedPosition(selectedGuti);
								
							greenIndex[selectedGuti]=selectedPosition;
							System.out.println("1st player selectedPosition "+selectedPosition);
							
							
								guti[selectedGuti].getFill();
								guti[selectedGuti].setCenterX(board[selectedPosition][0]);
								guti[selectedGuti].setCenterY(board[selectedPosition][1]);
	...
	public int selectTheNumberOfGuti()
	{		
		while(true)
		{			
			Random rand = new Random();
			int selectedGuti = rand.nextInt(3);
			int point = greenIndex[selectedGuti];
			
			for(int i=0;i<8;i++)
			{
				if(path[point][i]>=0
						&&board[path[point][i]][2]==0)
				{					
					return selectedGuti;
				}
	...
	public int selectedPosition(int selectedGuti)
	{		
		int point = greenIndex[selectedGuti];
		while(true)
		{			
			Random rand = new Random();
			int selectedPosition = rand.nextInt(8);
			
			if(path[point][selectedPosition]>=0
						&&board[path[point][selectedPosition]][2]==0)
			{					
					return path[point][selectedPosition];
			}
	...
	public void winCheckGreenGuti()
	{
		...
		{			
			...
			Stage3_2_L pane = new Stage3_2_L();
	...
	public void winCheckBlueGuti()
	{
		...
		{			
			...
			Stage3_2_W pane = new Stage3_2_W();
